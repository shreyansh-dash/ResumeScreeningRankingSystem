...
def extract_text(path: Path) -> str:
    try:
        ...
        raise HTTPException(
            422,
            "The resume could not be read. Use a text-based PDF or DOCX file."
        ) from exc
...
def analytics(
    ...
    if search:
        where.append(
            "(c.Name LIKE :search OR c.Job_Role LIKE :search OR "
            "EXISTS (SELECT 1 FROM historical_skills hs_search WHERE "
            "hs_search.Resume_ID=c.Resume_ID AND hs_search.Skill LIKE :search))"
        )
        values["search"] = f"%{search.strip()}%"
    ...
    summary = query_rows(
        f"""SELECT COUNT(*) AS candidates,
                  ...
                  ROUND(AVG(c.AI_Score),1) AS average_score
           FROM historical_candidates c
           WHERE {clause}""",
        values
    )[0]
    ...
    skills = query_rows(
        f"""SELECT c.Job_Role, hs.Skill, COUNT(*) AS frequency
           FROM historical_candidates c
           JOIN historical_skills hs ON hs.Resume_ID=c.Resume_ID
           WHERE c.Recruiter_Decision='Hire' AND {clause}
    ...
    salaries = query_rows(
        f"""SELECT c.Job_Role, ROUND(AVG(c.Salary_Expectation),0) AS average_salary
           FROM historical_candidates c
           WHERE {clause}
           GROUP BY c.Job_Role
           ORDER BY c.Job_Role""",
    ...
    score_decisions = query_rows(
        f"""SELECT CASE WHEN c.AI_Score >= 80 THEN 'High (80–100)'
                     WHEN c.AI_Score >= 50 THEN 'Medium (50–79)'
                     ELSE 'Low (0–49)'
                END AS score_band,
                SUM(c.Recruiter_Decision='Hire') AS hires,
           ...
           WHERE {clause}
           GROUP BY score_band
           ORDER BY MIN(c.AI_Score) DESC""",
        values
    )

    experience = query_rows(
        f"""SELECT c.Experience_Years, COUNT(*) AS candidates
           FROM historical_candidates c
           WHERE {clause}
...
async def screen_resume(
    ...
    if not role_exists:
        raise HTTPException(
            400,
            "Choose a target role available in the historical dataset."
        )
    ...
    avg_experience = sum(
        row["Experience_Years"] for row in hired
    ) / len(hired)
    avg_projects = sum(
        row["Projects_Count"] for row in hired
    ) / len(hired)
    avg_salary = sum(
        float(row["Salary_Expectation"]) for row in hired
    ) / len(hired)

    skill_score = (
        (len(matched) / len(benchmark_skills) * 100)
        if benchmark_skills
        else 0
    )
    experience_score = max(
        0,
        100 - abs(profile["experience_years"] - avg_experience)
        / max(avg_experience, 1) * 100
    )
    project_score = min(
        100,
        profile["projects_count"] / max(avg_projects, 1) * 100
    )
    education_score = (
        100
        if any(
            profile["education"].lower() in str(row["Education"]).lower()
            for row in hired
        )
        else 50
    )
    salary_score = (
        ...
        if expected_salary is None
            ...
            0,
            100 - abs(expected_salary - avg_salary)
            / max(avg_salary, 1) * 100
        )
    )
    ...
    feedback = (
        f"For {target_role}, prioritize "
        f"{', '.join(missing[:3]) or 'clearer evidence of role-specific work'}."
    )
    ...
    with engine.begin() as connection:
        ...
        for skill in profile["skills"]:
            connection.execute(
                text(
                    """INSERT IGNORE INTO live_upload_skills
                       (Upload_ID, Skill)
                       VALUES (:upload_id, :skill)"""
                ),
                {
                    "upload_id": upload_id,
                    "skill": skill
